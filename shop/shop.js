...
function updateCartTotal() {
    ...
    cartItems.forEach(item => {
        const itemPrice = parseFloat(item.querySelector('.cart-item-price').innerText.replace('Rs', '').replace(/,/g, ''));
        totalPrice += itemPrice;
    });

    const cartTotal = document.querySelector('.cart-total');
    cartTotal.innerText = `Total: Rs${totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")}`;
}
    ...
    for (let i = 0; i < quantity; i++) {
        ...
        cartItem.innerHTML = `
            ...
            <div class="item-details">
                ...
                <span class="cart-item-price">Rs${itemPrice}</span>
            </div>
